...
// El envío y el WhatsApp salen de `store_settings`, la misma tabla que lee
// el servidor al calcular el pedido. Antes eran constantes de `mockData`, así
// que cambiar el envío en la base hacía que la tienda mostrara un precio y el
// servidor cobrara otro.
const settings = useSettingsStore()

// El descuento por combo lo calcula la base, no este componente: una segunda
// implementación en JavaScript acabaría discrepando de la que cobra el
// servidor. Aquí solo se pregunta y se muestra.
const promos = usePromotionsStore()
const combo = ref(null)

async function refrescarCombo() {
  combo.value = await promos.comboParaCarrito(cart.items)
}
...
// El carrito cambia con cada botón de +/−, así que se vuelve a preguntar.
watch(() => cart.items.map((i) => `${i.id}:${i.quantity}`).join('|'), refrescarCombo)
const step = ref(1)
...
const descuentoCombo = computed(() => combo.value?.descuento || 0)
const aPagarProductos = computed(() => Math.max(cart.subtotal - descuentoCombo.value, 0))
// El envío se calcula sobre lo que se paga de verdad, igual que en el servidor.
const shipping = computed(() => settings.envioPara(aPagarProductos.value))
const total = computed(() => aPagarProductos.value + shipping.value)
